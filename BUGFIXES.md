...
- Fixed: Transfer ledger updates were using envelope IDs as if they were array positions. The transfer route now resolves the correct array indexes with `findIndex` before passing the matching envelope objects into `ledgerUpdate`, so transfer entries are written to the correct ledgers. [2026-04-11]
